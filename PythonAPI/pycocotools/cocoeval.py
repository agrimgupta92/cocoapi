...
class COCOeval:
    ...
    def _prepareCatGroups(self):
        ...
        for idx, _cat_data in enumerate(cat_data):
            ...
            cat_groups[self.params.imgCountLbl.index(frequency)].append(idx)
        return cat_groups
    ...
    def summarize(self):
        ...
        '''
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100, catGroupIdx=None):
            p = self.params
            iStr = ' {:<18} {} @[ IoU={:<9} | area={:>6s} | maxDets={:>3d} catIds={:>3s}] = {:0.3f}'
            ...
            catGroupName = self.params.imgCountLbl[catGroupIdx] if catGroupIdx is not None else 'all'
        ...
        def _summarizeDets():
            ...
            if self.islvis:
                ...
                stat_idx = 5
            stats[stat_idx + 1] = _summarize(0, maxDets=self.params.maxDets[0])
            stats[stat_idx + 2] = _summarize(0, maxDets=self.params.maxDets[1])
            stats[stat_idx + 3] = _summarize(0, maxDets=self.params.maxDets[2])
            stats[stat_idx + 4] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2])
            stats[stat_idx + 5] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[stat_idx + 6] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2])
            return stats
        def _summarizeKps():
            stats = np.zeros((10,))
